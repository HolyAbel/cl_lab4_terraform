...
resource "openstack_compute_instance_v2" "event_reminder_tg" {
  name        = "event_reminder_tg"
...
resource "openstack_compute_instance_v2" "event_reminder_db" {
  name        = "event_reminder_db"
  image_name  = var.image_name
  flavor_name = var.flavor_name
  key_pair = var.key_pair
  security_groups = [openstack_networking_secgroup_v2.sg.name]
